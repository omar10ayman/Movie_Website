...
export default function Movies() {
  ...
  const getMovies = async (pageNum)=>{
      let {data} = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=1bc6b1c83095f7fe5720985c1c6814b7&include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&include_adult=false&include_video=false&page=${pageNum}`)
  ...
  return (
   ...
   { movies? <div className='container'>
      <div className="row justify-content-center" id='om'>
        {movies.map(item=><Card key={item.id} id={item.id} title={item.title} poster_path={item.poster_path}/>)}
      </div>
    </div> :<Loading/>}
